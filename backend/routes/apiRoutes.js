...
router.post("/jobs", jwtAuth, (req, res) => {
  ...
  var transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: 'no.reply.careergrip@gmail.com',
      pass: '#seproj1234'
    }
  });
  ...
  User.find({}, function (err, users) {
    if (err) {
      ...
      for (var i = 0; i < users.length; i++) {
        if (users[i].type == "applicant") {
          var dur;
          if (job.duration == 0){
            dur = "Not Mentioned";
          }
          else{
            dur = job.duration.toString() + ' months';
          }
          var mailOptions = {
            from: 'no.reply.careergrip@gmail.com',
            to: users[i].email,
            subject: 'Career Grip - New Position for ' + job.title,
            html: '<h2>Check out this new postion for ' + job.title + '</h2><p>Deadline for Applying: ' + job.deadline
              + '</p><p>Duration: ' + dur + '</p><p>Salary: ' + job.salary + '</p><p>Skills: ' + job.skillsets
              + '</p><p>Job Type: ' + job.jobType + '</p><p>Only ' + job.maxPositions + ' Position(s) availaible!</p>' 
              + '<h3>Checkout the <a href="http://localhost:3000">website</a> for applying</h3>',
          };
